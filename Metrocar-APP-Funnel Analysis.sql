...
FROM
  conversion_rates
WHERE
  step IN ('Signup', 'Ride_Requested', 'Ride_Completed', 'Reviews');
...
FROM
  conversion_rates
WHERE
  step IN ('Sign_Ups', 'Ride_Requests', 'Ride_Completed', 'Reviews');
...
total_users AS (
  ...
  FROM (
    ...
    UNION
    SELECT CAST(user_id AS TEXT) FROM reviews
  ) subquery
)
SELECT
  funnel.step,
...
ORDER BY
  CASE
    WHEN funnel.step = 'App_Downloads' THEN 1
    WHEN funnel.step = 'Sign_Ups' THEN 2
    WHEN funnel.step = 'Ride_Requests' THEN 3
    WHEN funnel.step = 'Completed_Rides' THEN 4
    WHEN funnel.step = 'Reviews' THEN 5
  END;

/* create general funnel (no filters) aggregated dataset only with absolute values */

WITH funnel AS (
  SELECT
    1 AS funnel_step,
    'Downloads' AS funnel_name,
    COUNT(DISTINCT app_downloads.app_download_key) AS user_count,
    NULL::bigint AS ride_count
  FROM
    app_downloads
  LEFT JOIN
  ...
  WHERE
    ride_requests.user_id IS NOT NULL
  ...
  SELECT
    ...
    COUNT(DISTINCT ride_requests.user_id) AS user_count,
    COUNT(DISTINCT transactions.ride_id) AS ride_count
  FROM
    app_downloads
  LEFT JOIN
    signups ON app_downloads.app_download_key = signups.session_id
  ...
  LEFT JOIN
    ride_requests ON signups.user_id = ride_requests.user_id
  LEFT JOIN
    reviews ON ride_requests.ride_id = reviews.ride_id
  WHERE
    reviews.ride_id IS NOT NULL
)
SELECT
  funnel_step,
  funnel_name,
...
ORDER BY
  ...
  SELECT
    ...
    signups.age_range,
    DATE_TRUNC('day', app_downloads.download_ts) AS download_date,
    COUNT(DISTINCT app_downloads.app_download_key) AS user_count,
    NULL::bigint AS ride_count
  FROM
    app_downloads
  LEFT JOIN
    signups ON app_downloads.app_download_key = signups.session_id
  GROUP BY
    app_downloads.platform, signups.age_range, DATE_TRUNC('day', app_downloads.download_ts)
...
SELECT
  ...
  COALESCE(SUM(ride_count), 0) AS ride_count
FROM
  funnel
GROUP BY
  funnel_step, funnel_name, platform, age_range, download_date
ORDER BY
  download_date, funnel_step, platform, age_range;
